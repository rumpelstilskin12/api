...
public class InfosDAO extends DAO <Infos> {
    ...
    public Infos create(Infos obj) throws SQLException {
         String req1 = "insert into api_infos(idform,idsesscours,nb) values(?,?,?)";
        ...
        try (PreparedStatement pstm = dbConnect.prepareStatement(req)) {
            ...
            try (ResultSet rs = pstm.executeQuery()) {
                if (rs.next()) {
                    ...
                    int nbr = rs.getInt("NBRHEURE");
                    

                    return new Infos(idinfos, idform, idsesscours, nbr);
                } else {
    ...
    public Infos update(Infos obj) throws SQLException {
         String req = "update api_infos set idform=?,idsesscours=?,nbr=?"
                + " where id_infos=?";
        try (PreparedStatement pstm = dbConnect.prepareStatement(req)) {
